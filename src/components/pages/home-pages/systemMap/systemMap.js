...
const SystemMapComponent = ({ projectInfo }) => {
        ...
        if (ajaxDataInstance != null && ajaxDataInstance.length > 0) {
            ...
            if (listData != "") {
                for (var i = 0; i < listData.length; i++) {
                    ...
                    var deviceIn = listData[i].split(":")[1].split("*")[29];
                    ...
                    if (deviceType == 1 || deviceType == 9) {
                        ...
                        var dataT = d3.select("text[id=" + METER_LOAD.ID_METER_T_VALUE + deviceId + "]");
                        var dataLoadLevel = d3.select("text[id=" + METER_LOAD.ID_METER_LOAD_LEVEL_VALUE + deviceId + "]");
            ...
            <div>
                <div>
                    <div className="project-infor" style={{ padding: "8px 10px", display: "block" }}>
                        <span className="title-up-oneline text-left text-uppercase"><div style={{ marginTop: "-21px", color: "white" }}><i className="fa-solid fa-diagram-project ml-1" style={{ color: "#fff" }}></i> {projectInfo}{infor}</div></span>
                    </div>
                </div>
                ...
                {
                    beforeId > 0 ?
                        <Link className="system-map system-up mt-4" to={`/${customerId}/${urlSearch}/${type}/${projectId}/${beforeId}`}><img className="system-map-to-window" src="/resources/image/icon-arrow-up.png" alt="system-map-up" /></Link>
                        :
                        <Link className="system-map system-up mt-4" to={`/`} onClick={(event) => event.preventDefault()}><img className="system-map-to-window" src="/resources/image/icon-arrow-up-disable.png" alt="system-map-up" /></Link>
                }

                {
                    nextId > 0 ?
                        <Link className="system-map system-down mt-4" to={`/${customerId}/${urlSearch}/${type}/${projectId}/${nextId}`}><img className="system-map-to-window" src="/resources/image/icon-arrow-down.png" alt="system-map-down" /></Link>
                        :
                        <Link className="system-map system-down mt-4" to={`/`} onClick={(event) => event.preventDefault()}><img className="system-map-to-window" src="/resources/image/icon-arrow-down-disable.png" alt="system-map-down" /></Link>
                }

                <Link className="system-map system-reset mt-4" to={`/`} onClick={(event) => event.preventDefault()}><img id="reset-zoom" className="system-map-to-window" src="/resources/image/icon-reset-zoom.png" alt="reset-zoom" /></Link>
                <div id="grid-canvas" className="grid-canvas-home">
...
export default SystemMapComponent;