...
const Chart = () => {
    ...
    const funcSetSystemTypeId = (e) => {
        ...
        funcGetDataLoad(projectId, type, typeTime, fromDate, toDate, activeButton, systemTypeId, deviceId, null, null, undefined)
    }
    ...
    const checkAuthorization = async (projectId, type, typeTime, fromDate, toDate, option, systemTypeId) => {
        ...
        if (check) {
            if (role === "ROLE_ADMIN") {
                funcGetDataLoad(projectId, type, typeTime, fromDate, toDate, option, systemTypeId, "", null, null, undefined)
            }
            if (role === "ROLE_MOD") {
                let customerIds = ""
                let projIds = ""
    ...
    const funcGetDataLoad = async (projectId, type, typeTime, fromDate, toDate, option, systemTypeId, deviceId, dataSankeyClass, deviceName, dataDeviceName, dataDeviceSelect) => {
        ...
        if (type == 1 || type == 0) {
            ...
            if (res.status === 200) {
                ...
                setIsLoading(false);
                drawChartEnergy(res.data.dataLoadAllSite, res.data.dataComp, resProject.data, projectId, typeTime, resDevice.data, dataDeviceName);
            }
            ...
            if (projectId != 0) {
                ...
                $.alert({
                    title: t('content.notification'),
                    content: "Vui lòng chọn dự án để xem nhiệt độ",
                });
            }
        } else if (type == 6) {
            if (projectId != 0) {
                ...
                $.alert({
                    title: t('content.notification'),
                    content: "Vui lòng chọn dự án để xem sankey",
                });
            }
        } else if (type == 7) {
            if (projectId != 0) {
                ...
                $.alert({
                    title: t('content.notification'),
                    content: "Vui lòng chọn dự án để xem mức phóng điện",
                });
            }
        } else if (type == 8) {
            if (projectId != 0) {
                ...
                $.alert({
                    title: t('content.notification'),
                    content: "Vui lòng chọn dự án để xem kế hoạch năng lượng",
                });
            }
        } else if (type == 9) {
            ...
            if (res.status === 200) {
                ...
                setDataTablePower(res.data.dataNow);
            }
            setIsLoading(false);
        }
    }
    ...
    const drawChartSankey = (dataTotal, dataSankeyClass) => {
        ...
        if (dataTotal != null) {
            ...
            if (dataSankeyLocal != null) {
                if (dataSankeyLocal.name == userName) {
                    data = dataSankeyLocal.list;
                }
            }
        }
        const dataLocal = {
    ...
    const drawChartEnergy = (data, dataComp, projects, projectId, typeTime, dataDevice, dataDeviceName) => {
        const listCssColumn = [
            { width: 15, dx: -10 },
            { width: 30, dx: -15 },
            { width: 110, dx: -50 },
        ...
        switch (typeTime) {
            ...
            case 2:
                ...
                nowDate = "Tháng này"
                beforeDate = "Tháng trước";
                break;
            case 3:
                dataTimeX = dataTime.months
                nowDate = "Năm nay"
                beforeDate = "Năm trước";
                break;
            case 4:
                dataTimeX = data
                nowDate = "Tổng năm nay"
                beforeDate = "Tổng năm trước";
                break;
            case 5:
                dataTimeX = data
                nowDate = "Hôm nay"
    ...
    const drawChartEnergyComparison = (data) => {
        ...
        if (element != null) {
            ...
            chart.children.unshift(am5.Label.new(root, {
                text: "Biểu đồ so sánh năng lượng".toUpperCase(),
                fontSize: 28,
                fontWeight: "700",
                textAlign: "center",
                x: am5.percent(50),
    ...
    const funcSetType = (typeSelected) => {
        ...
        if (projectId == 0) {
            if (allowedTypeDevice.includes(typeSelected)) {
                $.alert({
                    title: t('content.notification'),
                    content: "Vui lòng chọn dự án để xem biều đổ này",
                });
                check = true;
            }
        }
                                ...
                                <div className="float-left" style={{ with: "75%" }}>
                                    <div style={{ position: "relative" }}>
                                        <div className="option-time">
                                            ...
                                            <button hidden={type == 8 ? true : false} className={activeButton == 5 ? "button-time" : "button-time-block"} onClick={() => onChangeValueEnergy(5)}>Tùy chỉnh</button>
                                        ...
                                        <table className="table-parameter" style={{ height: "400px", overflow: "auto", width: "100%" }} ref={tableRef}>
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th className={"th-content-last"}>TỔNG {unit}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
