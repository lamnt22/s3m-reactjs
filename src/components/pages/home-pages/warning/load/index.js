...
const WarningLoad = ({ customerId, projectId, systemTypeId, deviceId, updateCountStatus }) => {
    ...
    const drawChart = (dataWarning, settingList, settingValue, warningType) => {
        ...
        am5.ready(function () {
            ...
            function createSeriesHistory(name, field) {
                ...
                xAxisHistory.data.setAll(settingList);
                ...
                chart.yAxes.getIndex(0).set("separateStacks", true);
                series.data.setAll(settingList);
                series.appear(1000);
                chart.appear(1000, 100);
            }
            ...
            if (warningType == CONS.WARNING_TYPE.SONG_HAI_DONG_DIEN_BAC_N) {
                createSeries(t('content.home_page.warning_tab.setting_value'), 'settingValue')
                for (let x = 0; x < settingValue.length; x++) {
                    createSeriesN("Ngưỡng " + (x + 1), settingValue[x])
                }
            } else {
                if (settingList.length > 1) {
                    createSeriesHistory(t('content.home_page.warning_tab.setting_value'), 'settingValueHistory', false, "");
                ...
                } else if (warningType == CONS.WARNING_TYPE_MST.TONG_MEO_SONG_HAI_DIEN_AP || warningType == CONS.WARNING_TYPE_MST.TONG_MEO_SONG_HAI_DONG_DIEN) {
                            ...
                            <div className="infor" >
                                ...
                                <i className="fa-solid fa-circle fa-2xs"> &nbsp;</i>    <label>{t('content.home_page.warning_tab.warning_value')}: &nbsp;</label>{inforWarning.value} <br />
                                {
                                    device.warningType != 108 ?
                                        <>
                                            <i className="fa-solid fa-circle fa-2xs"> &nbsp;</i>    <label>{t('content.home_page.warning_tab.setting_value')}: &nbsp;</label>{inforWarning.settingValue.toString()}<br />
                                        </> : <>
                                            {
