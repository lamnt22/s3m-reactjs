...
const ManufactureTableData = (props) => {
    ...
    const getListDate = (startDate, endDate) => {
        let result = [];
        ...
        if (props.manufature.devices != undefined) {
            result.map(date => {
                getEpByShiftAndViewTime(param.customerId, props.manufature.projectId, moment(date).format("YYYY-MM-DD"), convertIdDevices(props.manufature.devices), convertShiftId(props.settingShift));
            })
        }
        if (props.manufature.id != undefined) {
            getListManufactureDetail(param.customerId, props.manufature.id, moment(fromDate).format("YYYY-MM-DD"), moment(toDate).format("YYYY-MM-DD"));
        }
    }
    ...
    const checkEpByViewTime = (ep) => {
        let check = dataEp.filter((d) => d.shiftId == ep.shiftId && d.viewTime == ep.viewTime)[0];
        if (dataEp.length > 0) {
            if (check == undefined) {
                dataEp.push(ep);
            }
    ...
    const getEpByShiftAndViewTime = async (customer, project, viewTime, devices, shiftIds) => {
        let res = await ManufactureService.getDataEpByShiftAndViewTime(customer, project, viewTime, devices, shiftIds);
        if (res.status == 200) {
            res.data.map((d) => {
                checkEpByViewTime(d);
            })
    ...
    const getDataEp = (dataEps, date, shiftId) => {
        if (dataEps.length > 0) {
            let a = dataEp.filter((data) => data.shiftId == shiftId && moment(data.viewTime).format("DD-MM-YYYY") == moment(date).format("DD-MM-YYYY"))[0]
            return a;
        }
    }
    ...
    return <>
        <div style={{ position: 'relative' }}>
            ...
            <table className="hihi" width={"98%"} >
                ...
                {/* {props.manufactureShift.length <= 0 ?
                    <tbody>
                        <tr>
                            <td colSpan={4 + props.settingShift.length}>Không có dữ liệu</td>
                        </tr>
                    </tbody>
                    : */}
                    ...
                    {dates.map((date, index) => {
                        ...
                        return <React.Fragment key={"manu" + index}>
                            ...
                            <tr height="50px" style={{ fontWeight: 'bold' }}>
                                {props.settingShift.map((shift, index) => {
                                    return (
                                        <td key={index + 'e'}>{getDataEp(dataEp, date, shift.id) != undefined ? getDataEp(dataEp, date, shift.id).epTotal : 0} kWh </td>
                                        // <td key={index + 'e'}> kWh </td>
                                    )
                                })}
                            ...
                            <tr height="50px" style={{ fontWeight: 'bold' }}>
                                {props.settingShift.map((shift, index) => {
                                    return (
                                        <td key={index + 'e'}>{getDataEp(dataEp, date, shift.id) != undefined ? (getDataEp(dataEp, date, shift.id).lowCost + getDataEp(dataEp, date, shift.id).normalCost + getDataEp(dataEp, date, shift.id).highCost) : 0} VND </td>
                                        // <td key={index + 'e'}> kWh </td>
                                    )
                                })}
