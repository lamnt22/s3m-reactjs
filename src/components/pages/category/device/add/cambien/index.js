...
const CambienAny = (props) => {
    const initialValues = {
        ...
        fuelTypeId: props.data.fuelTypeId,
        model: "",
        dbId: "",
        description: "",
        manufacturer: '',
    ...
    const keepValueForm = () => {
        ...
        formik.setFieldValue("battery_capacity", $('#battery_capacity').val());
            ...
            <div id="formValue" className={`relative flex flex-col items-center justify-center mt-5 p-2 
                ...
                <table className={`table table-input `}>
                    <tbody>
                        ...
                        <tr className="battery_capacity">
                            <th>{t('content.category.device.battery_capacity')} (mAh)</th>
                            <td>
                                <input type="number" className="form-control" name="battery_capacity" id="battery_capacity" defaultValue={formik.values.battery_capacity} onChange={formik.handleChange} />
                            </td>
                        </tr>
...

            {
                props.isActive == 4 ?
                    ...
                    bg-white rounded-md text-sm  ${props.isActive != 4 && "hidden"}`}>
                        <form onSubmit={formik.handleSubmit} id="form8">
                            <table className={`table table-input `}>
                                <tbody>
                                    <tr>
                                    ...
                                    <tr className="battery_capacity">
                                        <th>{t('content.category.device.battery_capacity')} (mAh)</th>
                                        <td>
                                            <input type="number" className="form-control" name="battery_capacity" id="battery_capacity" onChange={formik.handleChange} defaultValue={formik.values.battery_capacity} disabled />
                                        </td>
                                    </tr>
                                    <tr className="reference_device_id">
                                        <th>{t('content.category.device.connect_device')}</th>
                                    ...
                                    <tr>
                                        <th width="180px">Loại nhiên liệu</th>
                                        <td>
                                            <select className="custom-select fuelTypeId" id="fuelTypeId" onChange={formik.handleChange} defaultValue={props.data.fuelTypeId} disabled>
                                                <option value=""></option>
                                                {materialTypes.map((item, index) => (
                                                    <option key={index + 1} value={item.id}>{item.materialName}</option>
                                                ))}
                                            </select>
                                        </td>
                                    </tr>
